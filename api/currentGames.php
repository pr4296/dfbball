...
$query = "SELECT h.teamName as homeTeamName, h.city as homeCity, a.teamName as awayTeamName, a.city as awayCity, g.* FROM game g inner join team h on h.id = g.homeTeamId inner join team a on a.id = g.awayTeamId WHERE DATE(DATE_SUB(startTime, INTERVAL 6+24*(".$dayDiff.") HOUR)) = DATE(DATE_SUB(now(), INTERVAL 6+24*(".$dayDiff.") HOUR))  ".$and." ORDER BY startTime asc, awayTeamId asc";
// echo $query."\n";
